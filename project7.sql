...
DROP TABLE shippingLoc_info CASCADE CONSTRAINTS;
...
DROP SEQUENCE SEQ_payment_payment_idx;
DROP SEQUENCE SEQ_pet_pet_idx;
DROP SEQUENCE SEQ_product_image_pdt_image_idx;
...
DROP SEQUENCE SEQ_SALES_DETAILS_sales_idx;
DROP SEQUENCE SEQ_shippingLoc_info_shiplocInfo_idx;
...
CREATE SEQUENCE SEQ_cart_cart_idx INCREMENT BY 1 START WITH 1;
...
CREATE SEQUENCE SEQ_payment_payment_idx INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_pet_pet_idx INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_product_image_pdt_image_idx INCREMENT BY 1 START WITH 1;
...
CREATE SEQUENCE SEQ_SALES_DETAILS_sales_idx INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_shippingLoc_info_shiplocInfo_idx INCREMENT BY 1 START WITH 1;
...
(
	...
	-- 배송상태(PRP준비, dlv중, cmp완료)
	DELIVERY_STATUS char(3) NOT NULL,
	-- 결제 일련번호
	payment_idx number NOT NULL,
...
ALTER TABLE SALES_DETAILS
	ADD FOREIGN KEY (payment_idx)
	REFERENCES payment (payment_idx)
;


ALTER TABLE shippingLoc_info
	ADD FOREIGN KEY (payment_idx)
	REFERENCES payment (payment_idx)
;
...
COMMENT ON COLUMN cart.cart_idx IS '장바구니 일련번호';
COMMENT ON COLUMN cart.product_idx IS '상품 일련번호';
COMMENT ON COLUMN cart.member_idx IS '회원번호';
COMMENT ON COLUMN cart.product_quanity IS '상품 수';
...
COMMENT ON COLUMN payment.payment_idx IS '결제 일련번호';
COMMENT ON COLUMN payment.merchant_uid IS '상품 아이디';
COMMENT ON COLUMN payment.amount IS '가격';
COMMENT ON COLUMN payment.pg IS 'pg사';
COMMENT ON COLUMN payment.payStus IS '결제상태(결제,환불) pay,rfn';
COMMENT ON COLUMN payment.pay_date IS '결제일';
COMMENT ON COLUMN payment.seller IS '판매자';
COMMENT ON COLUMN payment.productname IS '상품명';
COMMENT ON COLUMN payment.member_idx IS '회원번호';
...
COMMENT ON COLUMN product.product_idx IS '상품 일련번호';
...
COMMENT ON COLUMN product.product_regdate IS '상품 등록일';
COMMENT ON COLUMN product.product_category IS '상품 카테고리 (약,사료,굿즈) (gds, fed , mdc)카테고리 (약,사료,굿즈) (gds, fed , mdc)';
COMMENT ON COLUMN product.product_stock IS '상품 재고';
COMMENT ON TABLE product_image IS '상품이미지 테이블';
COMMENT ON COLUMN product_image.pdt_image_idx IS '상품 이미지 일련번호';
COMMENT ON COLUMN product_image.pdt_image_file IS '상품 이미지 파일이름';
COMMENT ON COLUMN product_image.product_idx IS '상품 일련번호';
...
COMMENT ON COLUMN review_board.review_flag IS '입양/시터/쇼핑몰 후기 플래그(adp,sit,shp) ';
...
COMMENT ON COLUMN review_board.product_idx IS '상품 일련번호';
...
COMMENT ON TABLE sale IS '할인 테이블';
COMMENT ON COLUMN sale.member_grade IS '멤버 등급(sit,adpt,nor)';
COMMENT ON COLUMN sale.discount_rate IS '할인율(20,10,0)';
COMMENT ON TABLE SALES_DETAILS IS '판매내역 테이블';
COMMENT ON COLUMN SALES_DETAILS.sales_idx IS '판매내역 일련번호';
COMMENT ON COLUMN SALES_DETAILS.product_quanity IS '판매수량';
COMMENT ON COLUMN SALES_DETAILS.product_idx IS '상품 일련번호';
COMMENT ON COLUMN SALES_DETAILS.member_idx IS '회원번호';
COMMENT ON COLUMN SALES_DETAILS.payment_idx IS '결제 일련번호';
COMMENT ON TABLE shippingLoc_info IS '배송지 정보 테이블';
COMMENT ON COLUMN shippingLoc_info.shiplocInfo_idx IS '배송지 정보 일련번호';
COMMENT ON COLUMN shippingLoc_info.recipient IS '수령인';
COMMENT ON COLUMN shippingLoc_info.recipient_phone IS '연락처';
COMMENT ON COLUMN shippingLoc_info.SHIPPING_LOCATION IS '배송지 주소';
COMMENT ON COLUMN shippingLoc_info.DELIVERY_STATUS IS '배송상태(PRP준비, dlv중, cmp완료)';
COMMENT ON COLUMN shippingLoc_info.payment_idx IS '결제 일련번호';
...
COMMENT ON COLUMN sitter.sit_starpoint IS '시터 총 별점';
COMMENT ON COLUMN sitter.sit_starcount IS '시터가 별점을 받은 횟수 ';
